...
if __name__ == '__main__':
    ...
    leg_uvb = ax.legend([line_fg, line_no_uvb],uvb_labels, loc=1, fontsize=12)
    ...
    for b, background in enumerate(backgrounds):
        ...
        nh_i = np.argmin(np.abs(nh_array - hdens))
        temp_min_i = np.argmin(np.abs(T_array - temp_min))
        temp_max_i = np.argmin(np.abs(T_array - temp_max))
        indices = [nh_i*len(T_array) + i for i in range(len(T_array))]

        temp_plot = T_array[temp_min_i:temp_max_i+1]

        fion_tbl = np.loadtxt(ion_table_file)[:, ions_start_from:]
 
        for i, num in enumerate(species_id):
            frac_plot = fion_tbl[:, num][indices][temp_min_i:temp_max_i + 1]
            plt.plot(temp_plot, frac_plot, ls=linestyles[b], c=colors[i], label=species[i])
        if b == 0:
    ...
    plt.xlabel(r'${\rm log} (T / {\rm K})$')
    plt.ylabel(r'${\rm log }(f_{\rm ion})$')
    plt.xlim(temp_min, temp_max)
    plt.ylim(-6, 0.5)

    plt.title(r'${\rm log} n_H = $'+str(hdens))
    plt.savefig('ion_fracs_temp_o.png')
    plt.close()
